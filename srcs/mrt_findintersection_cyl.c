...
static void			cyl_calc_dist_abcd(t_ray *ray, t_cyl *cyl, double *abcd)
{
	...
	math_vec3_minus(ray->orig, cyl->pos, vec_sc);
	math_vec3_outerprod(ray->dir, cyl->orien, oprd_dn);
	math_vec3_outerprod(vec_sc, cyl->orien, oprd_scn);
	abcd[0] = math_vec3_innerprod(oprd_dn, oprd_dn);
	abcd[1] = 2.0 * math_vec3_innerprod(oprd_dn, oprd_scn);
	abcd[2] = math_vec3_innerprod(oprd_scn, oprd_scn)
				- cyl->dia * cyl->dia / 4.0;
...
static int			cyl_calc_normvec(t_surface *surface, t_ray *ray, t_cyl *cyl)
{
	...
	math_vec3_minus(surface->pos, cyl->pos, vec_c2s);
	iprd_c2s_n = math_vec3_innerprod(vec_c2s, cyl->orien);
	if (iprd_c2s_n < -MRT_EPS || iprd_c2s_n > cyl->hei)
		return (1);
	math_vec3_applylen(cyl->orien, iprd_c2s_n, surface->normvec);
	math_vec3_minus(vec_c2s, surface->normvec, surface->normvec);
