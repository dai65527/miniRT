...
/*   Updated: 2020/11/12 11:59:18 by dnakano          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	calc_reflect_applyratio(double *color_vec, double *ratio_vec,
										double *res_vec)
{
...
static void	calc_reflect_amblight(t_surface *surface, t_amblight *amblight,
									double *color_vec)
{
...
static int	has_obstruction(t_surface *surface, t_light *light, t_scene *scene)
{
	double		vec_s2l[3];
	t_ray		ray_s2l;
	t_surface	obstsurf;
	...
	obstsurf = mrt_findintersection(&ray_s2l, scene);
	return (obstsurf.dist > MRT_EPS && obstsurf.dist < math_vec3_norm(vec_s2l));
}

static void	calc_reflect_light(t_ray *ray, t_surface *surface, t_light *light,
									double *color_vec)
{
	double	inprd_tmp;
	double	vec_s2l[3];
	...
	if ((inprd_tmp = math_vec3_innerprod(surface->normvec, vec_s2l)) < MRT_EPS)
	{
	...
	math_vec3_applylen(lightcolor_vec, inprd_tmp * light->ratio, color_vec);
	calc_reflect_applyratio(color_vec, reflect_ratio, color_vec);
	math_vec3_applylen(surface->normvec, 2.0 * inprd_tmp, vec_tmp);
	math_vec3_minus(vec_tmp, vec_s2l, vec_tmp);
	if ((inprd_tmp = -math_vec3_innerprod(ray->dir, vec_tmp)) < 0.0)
		return ;
	math_vec3_applylen(lightcolor_vec,
		SPECREFLEC_FACTOR * pow(inprd_tmp, SHININESS) * light->ratio, vec_tmp);
